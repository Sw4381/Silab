...
- **labnote 글자 크기(2026-08 개편)**: 크기 선택은 **px 절대값**(`LN_FONT_SIZES` 11~40px + '기본'). ⚠예전엔 `execCommand('fontSize', 1~7)`(=`<font size>`)을 썼는데 그건 **브라우저 기본 크기 기준 상대 단계**라 편집창 기본 크기(A±)와 따로 놀고 단계 폭도 들쭉날쭉했다 — 되돌리지 말 것. 지금은 `markSelection()`이 `fontSize 7`로 선택 영역만 정확히 감싸고(부분 선택이면 브라우저가 바깥 서식 span을 알아서 쪼갬) 그 크기값을 즉시 지운 뒤 `font-size:Npx`를 넣는다(`applyFontSizePx`). '기본'(`clearFontSize`)은 크기 지정을 걷어내 A± 기본 크기를 따르게 하며, 예전 `<font size>` 태그도 같이 정리한다(`stripSizeInside`). 커서 위치의 크기는 `syncFontSel()`이 선택칸에 반영(목록에 없는 px은 임시 option으로 표시, 예전 태그 위에서는 `.legacy` 주황 테두리). 셀렉트를 클릭하면 편집창 선택이 풀리므로 `mousedown`에서 Range를 저장해 `change` 때 복원한다. 직접 DOM을 고치므로 input 이벤트가 안 떠서 `rtSync()`를 수동 호출해야 저장이 예약된다. A±(`ln_edfs`)는 **크기를 지정하지 않은 글자에만** 걸리는 기본 크기(기기별 localStorage).
